...
export class DrawingAndWritingComponent implements OnInit {
  ...
  updatedDefaultItemsProperties = {...this.defaultItemProperties, backgroundType: ''}
  ...
  item: DrawAndWritingModel = new DrawAndWritingModel();
  ...
  ngOnInit(): void {
    if (!this.selectedItemType) {
      this.selectedItemType = 'DRAWING_AND_WRITING';
    }
    ...
    this.scoringType = Object.values(ScoringTypeEnum).filter(
      (item) => ScoringTypeEnum.EXACT_MATCH == item
    );
    this.moderationStatus = this.itemService.currentSubjectModerationEnabled;
    this.matchingRule = Object.values(MatchingRuleEnums).filter(
      (item) => MatchingRuleEnums.EXACT_MATCH == item
    );
    ...
    if (this.editData) {
      ...
      this.defaultItemProperties.difficultyLevel = this.editData.difficultyLevel;
        // console.log(this.defaultItemProperties.difficultyLevel);
  ...
  buildItem(itemForm?: any):any {
    ...
    item.difficultyLevel = this.item.difficultyLevel ?? this.defaultItemProperties.difficultyLevel;
  ...
  saveFunction(item: any, type: string) {
    ...
    // console.log(item)
    this.itemService.createDrawWritingItem(item).subscribe(
      (value) => {
        //console.log(value);
